...
async def test_create_typification_success():
    ...
    with patch("lumina.services.typification_service.typification_repo.get_by_name", new_callable=AsyncMock, return_value=None), \
         patch("lumina.services.typification_service.typification_repo.add_typification"), \
         patch("lumina.services.typification_service.audit_service.register_action", new_callable=AsyncMock):
...
async def test_create_typification_conflict():
    ...
    with patch("lumina.services.typification_service.typification_repo.get_by_name", new_callable=AsyncMock, return_value=Typification(name="Nova Tipificacao")):
