...
from lumina.models import AccessType, User
...
@pytest.fixture
def project_mock(user):
    project = ProjectFactory.build(
        id=uuid4(), created_at=datetime.utcnow(), created_by=user.id
    )
...
def mock_project_repo(mocker):
    ...
    repo.has_project_access = AsyncMock(return_value=True)
...
async def test_create_project_success(
    mock_audit_service, mock_project_repo, session, user
):
    mock_project_repo.get_by_name.return_value = None
    data = ProjectCreate(name='New Project', description='Test')

    result = await project_service.create_project(session, user.id, data)
...
async def test_create_project_conflict(
    mock_project_repo, session, user, project_mock
):
    mock_project_repo.get_by_name.return_value = project_mock
    data = ProjectCreate(name='New Project')

    with pytest.raises(HTTPException) as exc:
        await project_service.create_project(session, user.id, data)
...
async def test_get_project_by_id_success(
    ...
    result = await project_service.get_project_by_id(
        session, user, project_mock.id
    )
    assert result == project_mock


@pytest.mark.asyncio
async def test_get_project_by_id_not_found(mock_project_repo, session, user):
    mock_project_repo.get_by_id.return_value = None

    with pytest.raises(HTTPException) as exc:
        await project_service.get_project_by_id(session, user, uuid4())

    assert exc.value.status_code == HTTPStatus.NOT_FOUND


@pytest.mark.asyncio
async def test_get_project_by_id_forbidden(
    mock_project_repo, session, user, project_mock
):
    mock_project_repo.get_by_id.return_value = project_mock
    mock_project_repo.has_project_access.return_value = False

    with pytest.raises(HTTPException) as exc:
        await project_service.get_project_by_id(session, user, project_mock.id)
...
async def test_update_project_success(
    mock_audit_service, mock_project_repo, session, user, project_mock
):
    mock_project_repo.get_by_id.return_value = project_mock
    mock_project_repo.get_by_name.return_value = None
    data = ProjectUpdate(id=project_mock.id, name='Updated Name')

    result = await project_service.update_project(session, user, data)
...
async def test_delete_project_success(
    ...
    await project_service.delete_project(session, user, project_mock.id)
