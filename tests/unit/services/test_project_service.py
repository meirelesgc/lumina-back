...
from lumina.schemas.project import ProjectCreate, ProjectUpdate, ProjectFilter
...
@pytest.mark.asyncio
async def test_create_project_success(mock_audit_service, mock_project_repo, session, user_id):
...
@pytest.mark.asyncio
async def test_create_project_conflict(mock_project_repo, session, user_id, project_mock):
...
@pytest.mark.asyncio
async def test_get_project_by_id_success(mock_project_repo, session, project_mock):
...
@pytest.mark.asyncio
async def test_update_project_success(mock_audit_service, mock_project_repo, session, user_id, project_mock):
    mock_project_repo.get_by_id.return_value = project_mock
    mock_project_repo.get_by_name.return_value = None
...
@pytest.mark.asyncio
async def test_delete_project_success(mock_audit_service, mock_project_repo, session, user_id, project_mock):
    mock_project_repo.get_by_id.return_value = project_mock
