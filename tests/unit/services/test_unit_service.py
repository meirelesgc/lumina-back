...
from lumina.schemas import UnitCreate, UnitUpdate, UnitFilter
...
async def test_create_unit_success(mock_audit, mock_repo, mock_session, user_id):
    # Arrange
...
async def test_create_unit_conflict(mock_repo, mock_session, user_id):
    # Arrange
    mock_repo.get_by_name = AsyncMock(return_value=Unit(name="Central", location="Other"))
    unit_data = UnitCreate(name="Central", location="Main Building")
...
@patch('lumina.services.unit_service.unit_repo')
@patch('lumina.services.unit_service.audit_service')
async def test_update_unit_success(mock_audit, mock_repo, mock_session, user_id):
    unit_id = uuid4()
