...
async def test_create_taxonomy_success(
    ...
    # Act
    result = await taxonomy_service.create_taxonomy(
        mock_session, user_id, tax_data
    )
...
async def test_create_taxonomy_conflict(mock_repo, mock_session, user_id):
    # Arrange
    typification_id = uuid4()
    mock_repo.get_conflict = AsyncMock(
    ...
    assert (
        exc.value.detail
        == 'Taxonomy title already exists for this typification'
    )
...
@patch('lumina.services.taxonomy_service.taxonomy_repo')
@patch('lumina.services.taxonomy_service.audit_service')
async def test_update_taxonomy_success(
    mock_audit, mock_repo, mock_session, user_id
):
    tax_id = uuid4()
    typification_id = uuid4()
    ...
    db_tax = Taxonomy(
        title='Old', description='Old Desc', typification_id=typification_id
    )
    db_tax.id = tax_id
    ...
    result = await taxonomy_service.update_taxonomy(
        mock_session, user_id, update_data
    )
