...
@pytest.mark.asyncio
async def test_create_source_success(mock_session, mock_source_repo, mock_audit_service):
    # Arrange
    user_id = uuid4()
...
async def test_create_source_conflict(mock_session, mock_source_repo):
    ...
    user_id = uuid4()
    data = SourceCreate(name="Test Source", description="Test Description")
    mock_source_repo.get_by_name.return_value = Source(name="Test Source", description="Desc")
...
@pytest.mark.asyncio
async def test_update_source_success(mock_session, mock_source_repo, mock_audit_service):
    # Arrange
    user_id = uuid4()
    source_id = uuid4()
    data = SourceUpdate(id=source_id, name="Updated Source", description="Updated Description")
...
@pytest.mark.asyncio
async def test_delete_source_success(mock_session, mock_source_repo, mock_audit_service):
    # Arrange
    user_id = uuid4()
    source_id = uuid4()
