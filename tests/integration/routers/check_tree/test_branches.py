...
from tests.factories.branch_factory import TypificationFactory, TaxonomyFactory, BranchFactory
...
async def test_get_branches_api(client, token, session, user, setup_taxonomy):
    branch = BranchFactory(title="List API Branch", taxonomy_id=setup_taxonomy.id)
...
@pytest.mark.asyncio
async def test_get_branch_by_id_api(client, token, session, user, setup_taxonomy):
    branch = BranchFactory(title="Get By ID API", taxonomy_id=setup_taxonomy.id)
    branch.set_creation_audit(user.id)
    session.add(branch)
    await session.commit()
