...
async def test_create_document_ai_message_endpoint(client, user, token):
    ...
    with patch('lumina.routers.docs.messages.message_service.create_message', new_callable=AsyncMock) as mock_create_msg:
        with patch('lumina.routers.docs.messages.message_service.list_messages', new_callable=AsyncMock) as mock_list_msg:
            with patch('lumina.routers.docs.messages.ai_service.create_ai_response', new_callable=AsyncMock) as mock_create_ai:
                ...
                def fake_create_message(*args, **kwargs):
                    msg_create = args[3] if len(args) > 3 else kwargs.get("msg") or kwargs.get("data")
                ...
                response = client.post(
                    ...
                    json={
                        'content': 'Onde a matriz foi aplicada?'
                    }
                )
