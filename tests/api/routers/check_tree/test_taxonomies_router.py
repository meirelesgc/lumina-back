...
async def test_read_taxonomies_router(client, token, session, typification):
    tax = Taxonomy(title="Existing Taxonomy", description="", typification_id=typification.id)
    session.add(tax)
    await session.commit()
...
@pytest.mark.asyncio
async def test_read_taxonomy_by_id_router(client, token, session, typification):
    tax = Taxonomy(title="Get Taxonomy", description="", typification_id=typification.id)
    session.add(tax)
    await session.commit()
...
async def test_delete_taxonomy_router(client, token, session, typification):
    tax = Taxonomy(title="Delete Taxonomy", description="", typification_id=typification.id)
    session.add(tax)
    await session.commit()
    await session.refresh(tax)
