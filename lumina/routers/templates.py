...
# Rotas de CRUD de resultados declaradas antes de /{template_id} para evitar colisao de UUID


@router.get('/results', response_model=ProcessingResultList)
async def list_template_compliance_results(
    session: Session,
    current_user: CurrentUser,
    filters: Annotated[ConformityFilter, Depends()],
):
    results = await template_conformity_service.list_results(session, filters)
    total = await template_conformity_service.count_results(session, filters)
    return {'count': total, 'results': results}


@router.get('/results/{result_id}', response_model=ProcessingResult)
async def get_template_compliance_result_by_id(
    result_id: UUID,
    session: Session,
    current_user: CurrentUser,
):
    return await template_conformity_service.get_result_by_id(
        session, result_id
    )
...
async def delete_template_compliance_result(
    result_id: UUID,
    session: Session,
    current_user: CurrentUser,
):
    await template_conformity_service.delete_result(
...
async def process_template_compliance(
    ...
    return await template_conformity_service.start_analysis(
        session,
        current_user.id,
        doc_id,
        file.filename,
...
async def list_templates(
    ...
    templates = await publication_template_service.get_templates(
        session, filters
    )
...
@router.post(
    ...
    response_model=PublicationTemplatePublic,
)
...
async def get_template(
    ...
    return await publication_template_service.get_template_by_id(
        session, template_id
    )
...
async def delete_template(
    ...
    await publication_template_service.delete_template(
