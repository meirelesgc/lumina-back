...
from lumina.features.template_abnt import pdf_metrics as m
from lumina.features.template_abnt.image_utils import render_page_png
...
def build_report(
    metadata: Metadata, secoes: list[SectionResult], description: str
) -> HybridReport:
...
def make_script_criterion(
    cid: str, title: str, checks: list[Check]
) -> Criterion:
    return Criterion(
        id=cid,
        title=title,
        match=all_match(checks),
        is_visual=False,
        checks=checks,
    )
...
def make_visual_criterion(
    ...
    return Criterion(
        id=cid, title=title, match=match, is_visual=True, criterios=criterios
    )
...
def build_page_size(tpl: dict, art: dict) -> Criterion:
    checks = [
        ...
        make_check(
            'altura',
            fmt_mm(tpl['height_mm']),
            fmt_mm(art['height_mm']),
...
def build_columns(tpl: dict, art: dict) -> Criterion:
    checks = [
        make_check(
            'numero_colunas',
            str(tpl['column_count']),
            str(art['column_count']),
            tpl['column_count'] == art['column_count'],
        ),
        make_check(
            'espaco_entre_colunas',
            fmt_mm(tpl['gutter_mm']),
            fmt_mm(art['gutter_mm']),
...
def build_main_font(tpl: dict, art: dict) -> Criterion:
    checks = [
        ...
        make_check(
            'tamanho',
            fmt_pt(tpl['font_size_pt']),
            fmt_pt(art['font_size_pt']),
            within(tpl['font_size_pt'], art['font_size_pt'], FONT_SIZE_TOL_PT),
        ),
        make_check(
            'bold',
            fmt_bool(tpl['font_bold']),
            fmt_bool(art['font_bold']),
    ...
    return make_script_criterion(
        'main_font', 'Fonte principal do texto', checks
    )
...
def build_line_spacing(tpl: dict, art: dict) -> Criterion:
    ...
    leading_ok = (
        tpl_lead is not None
        and art_lead is not None
        and within(tpl_lead, art_lead, SPACING_TOL_MM)
    )
    checks = [
        make_check(
            'modo', tpl['spacing_mode'], art['spacing_mode'], leading_ok
        ),
        make_check(
            'leading',
            fmt_mm(tpl_lead) if tpl_lead else 'indeterminado',
            fmt_mm(art_lead) if art_lead else 'indeterminado',
            leading_ok,
        ),
        make_check(
            'razao',
            fmt_ratio(tpl['spacing_ratio']),
            fmt_ratio(art['spacing_ratio']),
            leading_ok,
        ),
    ]
    return Criterion(
...
def build_references_typography(
    tpl_ref: dict | None, art_ref: dict | None
) -> Criterion:
    if not tpl_ref or not art_ref:
        checks = [
            make_check(
                'secao_localizada',
                fmt_bool(bool(tpl_ref)),
                fmt_bool(bool(art_ref)),
                bool(tpl_ref) == bool(art_ref),
            )
        ]
        return make_script_criterion(
            'references_typography',
            'Formatação tipográfica das referências',
            checks,
        )
    checks = [
        make_check(
        ...
        make_check(
            'tamanho_fonte',
            fmt_pt(tpl_ref['font_size_pt']),
            fmt_pt(art_ref['font_size_pt']),
            within(
                tpl_ref['font_size_pt'],
                art_ref['font_size_pt'],
                FONT_SIZE_TOL_PT,
            ),
        ),
        make_check(
            'alinhamento',
            tpl_ref['text_alignment'],
            art_ref['text_alignment'],
            tpl_ref['text_alignment'] == art_ref['text_alignment'],
        ),
        make_check(
            'estilo_numeracao',
            tpl_ref['numbering_style'],
            art_ref['numbering_style'],
...
def body_candidate_pages(doc, ref_bounds: dict | None) -> list[int]:
    ...
    if ref_bounds:
        exclude |= set(
            range(ref_bounds['start_page'], ref_bounds['end_page'] + 1)
        )
...
def run_script_page2(
    template_path: str, article_path: str, art_bounds: dict | None
) -> tuple[list[Criterion], list[int]]:
    tpl_doc = m.open_document(template_path)
    art_doc = m.open_document(article_path)
    try:
        ...
        art = m.aggregate_page_metrics([
            m.extract_page_metrics(art_doc, p) for p in candidate_pages
        ])
...
def run_script_references(
    template_path: str,
    article_path: str,
    tpl_bounds: dict | None,
    art_bounds: dict | None,
) -> tuple[list[Criterion], list[int], list[int]]:
    tpl_pages = [tpl_bounds['start_page']] if tpl_bounds else []
    art_pages = (
        list(range(art_bounds['start_page'], art_bounds['end_page'] + 1))
        if art_bounds
        else []
    )
    tpl_doc = m.open_document(template_path)
    ...
    try:
        tpl_ref = (
            m.extract_references_metrics(
                tpl_doc, first_page_bounds(tpl_bounds)
            )
            if tpl_bounds
            else None
        )
        art_ref = (
            m.extract_references_metrics(art_doc, art_bounds)
            if art_bounds
            else None
        )
    finally:
        ...
        art_doc.close()
    return (
        [build_references_typography(tpl_ref, art_ref)],
        tpl_pages,
        art_pages,
    )
...
def image_block(png_bytes: bytes, detail: str = VISION_DETAIL) -> dict:
    return {
        'type': 'input_image',
        'image_url': to_data_url(png_bytes),
        'detail': detail,
    }
...
def render_page_safe(
    path: str, page_number: int | None, dpi: int = VISION_DPI
) -> bytes | None:
    if not page_number:
        return None
    try:
...
async def visual_compare(
    client: AsyncOpenAI, template_png: bytes, article_png: bytes, label: str
) -> VisualComparisonResult:
    content = [
        text_block(f'TEMPLATE - {label}:'),
        image_block(template_png),
        text_block(f'ARTIGO - {label}:'),
        image_block(article_png),
    ]
    ...
    if result is None:
        raise RuntimeError(
            'Modelo de visão não retornou uma saída estruturada (possível refusal).'
        )
    return result
...
async def run_visual_check(
    ...
    try:
        ...
        return make_visual_criterion(
            ...
            [
                VisualCriterionItem(
                    criterio='Consulta ao modelo de visão',
    ...
    return make_visual_criterion(
        visual_id, visual_title, result.match, result.criterios
    )
...
async def run_pre_textual_task(
    client: AsyncOpenAI | None,
    template_path: str,
    article_path: str,
    tpl_page_count: int,
...
async def run_textual_task(
    ...
    visual, (script_criteria, art_p2_pages) = await asyncio.gather(
        ...
        asyncio.to_thread(
            run_script_page2, template_path, article_path, art_bounds
        ),
    )
    return build_section_result(
        SEC_TEXT,
        SEC_TITLES[SEC_TEXT],
        [tpl_p2] if tpl_p2 else [],
        art_p2_pages,
        visual,
        script_criteria,
    )
...
async def run_pos_textual_task(
    client: AsyncOpenAI | None,
    template_path: str,
    article_path: str,
    tpl_bounds: dict | None,
    ...
    (
        visual,
        (script_criteria, tpl_ref_pages, art_ref_pages),
...
async def compare(
    template_path: str, article_path: str, skip_visual: bool = False
) -> HybridReport:
