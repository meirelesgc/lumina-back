# Localização e amostragem de páginas (template/artigo): limites da seção de referências e páginas candidatas ao "corpo" do artigo.
...
def first_page_bounds(bounds: dict) -> dict:
    # Restringe os limites da seção de referências à PRIMEIRA página apenas (usado para o lado do template, que só entra na comparação pela 1a página).
    return {
...
def body_candidate_pages(doc, ref_bounds: dict | None) -> list[int]:
    # Todas as páginas do artigo, exceto a primeira e as pertencentes à seção de referências (usado para o scan determinístico da página 2).
    exclude = {1}
    if ref_bounds:
...
def compute_bounds(template_path: str, article_path: str) -> tuple[dict | None, dict | None, int, int]:
    # Abre os dois documentos uma única vez para descobrir os limites da seção de referências e a contagem de páginas; roda em thread (via asyncio.to_thread) antes de disparar as 3 tarefas de página, que depois abrem seus PRÓPRIOS handles de documento (PyMuPDF não é seguro para compartilhar um mesmo objeto Document entre threads concorrentes).
