...
from lumina.repositories import abnt_conformity_repo
from lumina.services import audit_service

logger = logging.getLogger(__name__)

SETTINGS = Settings()
DATA_DIR = Path(SETTINGS.STORAGE_DIRECTORY) / 'abnt_conformity'
...
async def list_results(
    ...
    return await abnt_conformity_repo.list_all(session, filters)


async def count_results(
    session: AsyncSession, filters: ConformityFilter
) -> int:
    return await abnt_conformity_repo.count(session, filters)


async def get_result_by_id(
    session: AsyncSession, result_id: UUID
) -> AbntConformityResult:
    record = await abnt_conformity_repo.get_by_id(session, result_id)
...
async def delete_result(
    ...
    await audit_service.register_action(
        ...
        user_id=user_id,
        action='DELETE',
        table_name=AbntConformityResult.__tablename__,
        record_id=record.id,
        old_data=old_data,
    )
...
async def _execute_analysis(
    ...
    record = await abnt_conformity_repo.get_by_id(session, analysis_id)
    if not record:
        return
    ...
    try:
        ...
        logger.exception(
            'Falha na verificação de conformidade ABNT (analysis_id=%s, doc_id=%s)',
            analysis_id,
            record.doc_id,
        )
        record.status = 'error'
...
async def run_analysis(
    ...
    if session is not None:
        await _execute_analysis(session, analysis_id, article_path)
    else:
        async with async_session() as bg_session:
            await _execute_analysis(bg_session, analysis_id, article_path)
...
async def start_analysis(
    ...
    abnt_conformity_repo.add(session, record)
    await session.flush()
